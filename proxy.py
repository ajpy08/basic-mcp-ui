"""
Proxy Web Server para la Interfaz MCP
Maneja las peticiones HTTP y actúa como proxy para las herramientas MCP
"""
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def handle_tool_call(self):
        """Maneja las llamadas a herramientas MCP"""
        try:
            ...
            # Importar las funciones core del servidor MCP
            try:
                from server import (
                    add_numbers, 
                    subtract_numbers, 
                ...
                logger.error(f"Error importing functions from server: {e}")
                response = {"error": "Servidor MCP no disponible"}
...
def start_web_server():
    """Inicia el servidor web proxy"""
    # Crear directorio estático si no existe
    if not os.path.exists(STATIC_DIR):
        os.makedirs(STATIC_DIR)
    
    server = HTTPServer((HOST_UI, PORT_UI), WebHandler)
    logger.info(f"Servidor web proxy iniciado en http://{HOST_UI}:{PORT_UI}")
    server.serve_forever()

def main():
    """Función principal para ejecutar solo el proxy"""
    start_web_server()
